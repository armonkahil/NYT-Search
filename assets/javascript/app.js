...
$("form").submit(function() {
  ...
  console.log(typeof(startYearEntry));
  console.log(startYearEntry.length);
  var endYearEntry = $("#endDate").val();
 //start year and end fields are not empty, these IF statements build the start and end year parts of the query
    if (startYearEntry.length > 0) {
        startYearEntry += "&begin_date="+ startFix;
    }
    if  (endYearEntry.length > 0) {
        endYearEntry += "&end_date="+ endFix;
    }

  var queryURL =
    "https://api.nytimes.com/svc/search/v2/articlesearch.json?q=" + searchTerm + startYearEntry + endYearEntry + "&api-key=" + apiKey;
  console.log(queryURL);
